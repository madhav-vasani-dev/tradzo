...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, StrategyCardComponent, DeployStrategyDialogComponent, ToastModule, ConfirmActionDialogComponent],
...
export class StrategiesComponent implements OnInit, OnDestroy {
  ...
  /** True while a deployment write is in flight — blocks a second deploy. */
  isDeploying = false;
  ...
  async onDeployed(config: DeployConfig) {
    // A deployment creates a userStrategy that trades real money — never run two at once.
    if (this.isDeploying) return;
